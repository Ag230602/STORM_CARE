...
METRIC_COLS = ["track_rmse", "L_data", "R_adv", "R_diff", "R_mass",
               "R_wp", "R_cont", "R_nrg"]  # TODO: confirm exact column names
                                           # in the validation CSV header
...
def run_one(tag: str, extra_args: list[str], epochs: int, dry: bool) -> Path:
    ...
    cmd = [PY, "-m", "model.physics.train", "--epochs", str(epochs),
...
def final_val_row(mdir: Path) -> dict:
    """Grab the last validation row. TODO: confirm the validation log
    filename the trainer writes (the audit references per-run val CSVs
    under metrics/physics/{full,no_physics}/)."""
    candidates = list(mdir.glob("*val*.csv")) or list(mdir.glob("*.csv"))
    if not candidates:
        return {}
...
def main() -> None:
    ...
    ap.add_argument("--epochs", type=int, default=50,
                    help="longer than the 20-epoch demo; log hardware")
    ...
    for seed in args.seeds:
        seed_args = ["--seed", str(seed)]  # TODO: confirm trainer flag name
