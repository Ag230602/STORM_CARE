...
- `results/module5_counterfactual/metrics/counterfactual_outcomes.csv` — baseline plus eight intervention outcomes
...
They are not injected directly into decoded trajectories. Earlier evacuation lowers peak exposure (`0.2915 -> 0.2831`), while delayed evacuation raises peak exposure (`0.2915 -> 0.2969`). Other infrastructure/resource interventions have weak or counterintuitive effects under the current short demo checkpoint, so they should be treated as model limitations rather than overclaimed planning recommendations.
