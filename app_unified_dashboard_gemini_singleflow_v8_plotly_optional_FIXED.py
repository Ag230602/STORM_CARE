...
def pydeck_map(
    ...
    # --- sanitize layer data for JSON serialization (avoid Timestamp/NA issues) ---
    def _sanitize_pydeck_data(obj):
        """Return JSON-friendly data for PyDeck (fixes Timestamp/NaT serialization)."""
        try:
        ...
        if isinstance(obj, _pd.DataFrame):
            ...
            for c in d.columns:
                ...
                if _pd.api.types.is_datetime64_any_dtype(s):
                    ...
                    if any(isinstance(x, (_pd.Timestamp, _dt.datetime, _dt.date, _np.datetime64)) for x in sample):
                        d[c] = s.astype(str)
            # replace NaN/NaT with None
            d = d.where(_pd.notnull(d), None)
            return d
        return obj

    for _layer in layers:
        try:
            _layer.data = _sanitize_pydeck_data(getattr(_layer, "data", None))
        except Exception:
            pass
...
def main():
    ...
    if areas is not None and len(areas)>0:
        ...
        with st.sidebar.expander('Areas grid preview'):
            st.dataframe(areas.head(20), use_container_width=True)
    ...
    with tab_path:
        ...
        if track is None:
            ...
            if _HAS_PLOTLY:
                st.plotly_chart(plotly_path_map(track, lead_sel if 'lead_sel' in locals() else None, center=center, zoom=zoom), use_container_width=True)
            else:
            ...
            if areas is None or len(areas) < 2:
                ...
                st.dataframe(show.astype(str), use_container_width=True)
                ...
                with g1:
                    ...
                    st.bar_chart(bars, x="Metric", y="Value", color="Spot", use_container_width=True)
                ...
                with g2:
                    ...
                    if rec is not None and "t" in rec.columns:
                        ...
                        st.line_chart(ts, x="t", y="pred_recovery", color="Spot", use_container_width=True)
                    else:
